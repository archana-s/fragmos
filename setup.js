    ...
    i.question("What is the name of your project? ", function (projectName) {
      i.question("Provide a local full path to your logo file (You can also leave it at default for now): ", function (logoLocation) {
        ...
        fs.mkdir('../styleguide', function (data) {
          copy('./src', '../styleguide/src', function (err, results) {
            if (err) {
              throw err;
            }

            // Add package.json and index.js files
            fs.createReadStream('package.json.toCopy').pipe(fs.createWriteStream('../styleguide/package.json'));
            fs.createReadStream('index.js').pipe(fs.createWriteStream('../styleguide/index.js'));

            // Include the project name in gulpfile
            var gulpFileData = fs.readFileSync('gulpfile.js', 'utf-8');
            ...
            copy('./public', '../styleguide/public', function (err, data) {
              if (err) throw err;
              ...
              console.log('You can now delete this directory. You generated styleguide + all styles are available in styleguide dir');
              i.close();
            });
          });
        });
